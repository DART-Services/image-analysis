...
public class ImageAnalyzerFactory implements AutoCloseable {
    ...
    /**
     * Defines system property that specifies the directory that contains the tesseract 
     * training and configuration data.  By default this will use {@code ./tessdata}.
     */
    public static final String DATAPATH = "org.dharts.dia.tesseract.datapath";
    public static final String LANGUAGE = "org.dharts.dia.tesseract.lg";
    public static final String MODE = "org.dharts.dia.tesseract.mode";

    public static ImageAnalyzerFactory createFactory() throws IOException, TesseractException 
    {
        // NOTE we need to be able to get this from a jar'd bundle, so we 
        String path = System.getProperty(DATAPATH, "./tessdata");
        String lg = System.getProperty(LANGUAGE, "eng");
        String mode = System.getProperty(MODE, OcrEngineMode.TESSERACT_ONLY.toString());
        
        File dir = new File(path);
        return createFactory(dir.getParentFile(), lg, OcrEngineMode.valueOf(mode));
    }
