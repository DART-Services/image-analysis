...
public class LayoutIterator {
     ...
     * 
     * <p>
     * NOTE that <code>{@link LayoutIterator.LevelDefinition.SYMBOL}</code> will skip non-text blocks, but all other 
     ...
     */
    public boolean next(TesseractLevel level) {
        return iterator.next(level.getTessCode());
    }
     ...
     */
    public boolean isAtBeginningOf(TesseractLevel level) {
        return iterator.isAtBeginningOf(level.getTessCode());
    }
     ...
     *      {@link LayoutIterator.LevelDefinition.PARA}.
     ...
     *      if the current word is the last word in a paragraph, this should be 
     *      {@link LayoutIterator.LevelDefinition.WORD}.
     ...
     */
    public boolean isAtFinalElement(TesseractLevel level, TesseractLevel element) {
        return this.iterator.isAtFinalElement(level.getTessCode(), element.getTessCode());
    }
     ...
     */
    public BoundingBox getBoundingBox(TesseractLevel level) {
        ...
        boolean exists = iterator.getBoundingBox(level.getTessCode(), left, top, right, bottom);
        return (exists) ? new SimpleBoundingBox(left.get(), top.get(), right.get(), bottom.get()) : null;
    }
    ...
    public Baseline getBaseline(TesseractLevel level) throws TesseractException {
        ...
        boolean success = iterator.getBaseline(level.getTessCode(), x1, y1, x2, y2);
     ...
     * <code>{@link LayoutIterator.LevelDefinition.BLOCK}</code>.
     * 
    ...
    public static class Baseline {
        ...
        public boolean equals(Object obj) {
            if (obj == null)
                return false;
            if (obj.getClass() != Baseline.class)
                return false;
            
            Baseline bo = Baseline.class.cast(obj);
            return x1 == bo.x1 && y1 == bo.y1
                && x2 == bo.x2 && y2 == bo.y2;
        }
